#
# Implements the DIS bound in theorem 4.6 using  KL divergence.
#
...
def DIS(gibbs_risk, disagreement, ng, nd, KL_QP, delta=0.05):
    g_rhs = ( KL_QP + log(4.0*sqrt(ng)/delta) ) / ng
...
def DIS_MV(gibbs_risk, disagreement, ng, nd, KL_QP, KL_rhopi, delta=0.05):
...
def compute_mv_loss(emp_risks_views, emp_dis_views, posterior_Qv, posterior_rho, prior_Pv, prior_pi, ng, nd, delta, lamb=None, gamma=None):
    """
...
def optimizeDIS_mv_torch(emp_risks_views, emp_dis_views, ng, nd, device, max_iter=1000, delta=0.05, eps=10**-9, optimise_lambda_gamma=False):
    """
    ...
    for i in range(max_iter):
        ...
        loss = compute_mv_loss(emp_risks_views, emp_dis_views, posterior_Qv, posterior_rho, prior_Pv, prior_pi, ng, nd, delta, lamb, gamma)
...
def compute_loss(emp_risks, emp_dis, posterior_Q, prior_P, ng, nd, delta, lamb=None, gamma=None):
    """
    ...
    if lamb is None or gamma is None:
        lamb = 2.0 / (torch.sqrt((2.0 * ng * emp_risk) / (KL_QP + torch.log(4.0 * torch.sqrt(ng) / delta)) + 1.0) + 1.0)
        gamma = torch.sqrt(2.0 * (KL_QP + torch.log((4.0*torch.sqrt(nd))/delta)) / (emp_disagreement*nd))
...
def optimizeDIS_torch(emp_risks, emp_dis, ng, nd, device, max_iter=1000, delta=0.05, eps=10**-9, optimise_lambda_gamma=False):
    """
    Optimize the value of `lambda` using Pytorch for Multi-View Majority Vote Learning Algorithms.
    ...
    for i in range(max_iter):
        ...
        # Calculating the loss
        loss = compute_loss(emp_risks, emp_dis, posterior_Q, prior_P, ng, nd, delta, lamb, gamma)
